...
const Coupon = (props) => {
    const {isLast, organizationColor} = props;
    const [organization, setOrganization] = useState(null);

    useEffect(() => {
        if (props.organization && !organizationColor) {
    ...
    const handleToCoupon = () => {
        props.navigation.navigate('OpenShareCompany', {
            couponId: props._id,
            isLoadingCoupon: true
        })
    }

    let color = organization?.colors?.primary || organizationColor || '';
