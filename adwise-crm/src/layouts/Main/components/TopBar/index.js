...
const TopBar = (props) => {
    const {onChangeVisibleDrawer, countNotification} = props;
    ...
    return (
        <Grid className={clsx({
            ...
            <Grid className={clsx([classes.elementHeader])} item>
                <Badge color="primary" badgeContent={countNotification}>
                    <Button className={clsx([classes.headerButton])} onClick={() => handleToRoute('/notifications')}>
                        <img src="/source/svg/header/notification.svg"/>
                    </Button>
                </Badge>
            </Grid>

            <Grid className={clsx([classes.elementHeader])} item>
                <Button className={clsx([classes.headerButton])} onClick={() => handleToRoute('/tariffs')}>
                    <HeaderPacket packet={props.organization.packet}/>
                </Button>
            </Grid>
...
const HeaderPacket = ({ packet }) => {
    ...
    if (!packet) {
        return (
            <>
                <ClockIcon/>
                <Typography className={classes.typographyBusiness}>Нет пакета</Typography>
            </>
        )
    }

    const finalDate = moment(packet.timestamp).add(packet.period || 12, 'month').format('DD.MM.YYYY');

    return (
        <>
            <ClockIcon/>
            <Typography className={classes.typographyBusiness}>{ packet.name } <span>до { finalDate }</span></Typography>
        </>
    )
}
...
export default compose(
    connect(
        state => ({
            ...
            countNotification: state.app.countNotification,
