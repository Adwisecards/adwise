...
const CashierAutocomplete = (props) => {
    ...
    useEffect(() => {
        ...
        ( async () => {
            await handleFindClients();
        })();
    }, [value]);

    const handleFindClients = async (search) => {

        const filter = `?page=1&limit=999&search=${ search }&order=-1&sortBy=user`;
        const response = await axiosInstance.get(`${ urls["get-employees"] }/${ global.organization._id }${ filter }`).then((res) => {
