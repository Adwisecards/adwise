...
const MyCompany = (props) => {
    ...
    if (organization && organization.employees){
        employeesCount = (organization.employees.length > 0) ? organization.employees.length - 1 : organization.employees.length;
    }

    return (
        <Box mb={5}>
            {/*<Box mb={2}>*/}
            {/*    <Typography className={classes.sectionTitle}>Моя компания</Typography>*/}
            {/*</Box>*/}
            ...
            {/*    isActive={handleIsOpen(['/coupons', '/shares/create', '/coupons/'])}*/}
            
            {/*    onClick={() => handleChangePage('/coupons')}*/}
            {/*    onClickAdd={() => handleChangePage('/shares/create')}*/}
            {/*    isDisabledSidebar={isDisabledSidebar}*/}
            {/*/>*/}
        </Box>
    )
